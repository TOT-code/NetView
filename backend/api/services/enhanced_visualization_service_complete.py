...
class EnhancedVisualizationService:
    ...
    def get_node_details(self, task_id: str, node_id: str) -> Optional[Dict[str, Any]]:
        ...
        output_edges = [edge for edge in edges if edge.get("source") == node_id]
        
        return {
            "node": target_node,
            "input_connections": input_edges,
            "output_connections": output_edges,
            "connection_summary": {
                "input_count": len(input_edges),
                "output_count": len(output_edges),
                "total_connections": len(input_edges) + len(output_edges)
            }
        }
    
    def get_subgraph(self, task_id: str, node_ids: List[str]) -> Optional[Dict[str, Any]]:
        """获取子图数据"""
        viz_data = self.get_visualization_data(task_id)
        if not viz_data:
            return None
        ...
        filtered_nodes = [node for node in all_nodes if node.get("id") in node_ids]
        ...
        # 计算子图统计
        subgraph_stats = self._calculate_subgraph_statistics(filtered_nodes, filtered_edges)
        ...
        return {
            ...
            "metadata": {
                ...
                "coverage_ratio": len(filtered_nodes) / max(len(all_nodes), 1)
            }
        }
    
    def _calculate_subgraph_statistics(self, nodes: List[Dict], edges: List[Dict]) -> Dict[str, Any]:
        """计算子图统计信息"""
        # 基础统计
        stats = {
        ...
        # 参数统计
        total_params = sum(node.get("num_parameters", 0) for node in nodes)
        total_flops = sum(node.get("flops", 0) for node in nodes)
        ...
        stats.update({
            ...
            "avg_parameters_per_node": total_params / max(len(nodes), 1),
